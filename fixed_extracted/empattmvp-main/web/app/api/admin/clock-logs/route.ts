 ...
 *     distance_from_site_meters, within_geofence, sequence_anomaly,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: logs, error } = await supabaseAdmin
      .from('clock_logs')
      .select(
        'id, event_type, latitude, longitude, accuracy_meters, is_mock_location, distance_from_site_meters, within_geofence, sequence_anomaly, client_timestamp, work_sites(name)'
      )
    ...
    const formatted = (logs ?? []).map((log) => ({
      ...
      sequence_anomaly: log.sequence_anomaly,
